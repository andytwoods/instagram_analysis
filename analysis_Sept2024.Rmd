...
load_data_per_cohort <-
  function(t0_file_nam, tx_file_nam, cohort_name) {
    ...
    combined <- combined |>
      ...
      dplyr::select(delay, date_of_test2, date_of_test1, days, media, duration_seconds, unique_sj_id,  age, ar_frequency,  study_name, cohort, exp_title_question, score, exp_title, enjoy, 
                    knew, surround, captivated, presence_beingthere, emotion_awe, emotion_boredom, presence_real, gap)
...
combined_data |>
  ...
  dplyr::summarize(
                   SJs=n_distinct(id), 
                   age_mean=sprintf("%.1f (%.1f)", mean(age*10) , sd(age*10))
                   ) |> 
  kable("html", 
...
fit.mod1 <-glmer(formula = score ~ 1 + media * time * gap +  (1  | cohort / exp_title / exp_title_question) + (1 + time | id) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum, time=contr.sum, gap=contr.sum), data=combined_new)
# age + presence  + knew + ar_frequency


fit.mod2 <-glmer(formula = score ~ 1 + media * time * gap + (1 + time | id) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum, time=contr.sum, gap=contr.sum), data=combined_new)



##### double analysis with t1 vs t2

fit.t1 <- combined_data_cor_screened |>
  filter(
    months != "t2"
  ) |>
  glmer(formula = score ~ 1 + months * media   +  (1  | cohort / exp_title / exp_title_question) + (1 | id) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum))


fit.t2 <- combined_data_cor_screened |>
  filter(
    months != "t1"
  ) |>
  glmer(formula = score ~ 1 + months * media +  (1  | cohort / exp_title / exp_title_question) + (1  | id) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum))

emmeans(fit.t1, pairwise~media|months, type='response')
emmeans(fit.t2, pairwise~media|months, type='response')

plot_model(fit.t1, type='int')
plot_model(fit.t2, type='int')

##### repeat t2 analysis with tripple incl. cohort 

fit.t.t2_split <- combined_data_cor_screened |>
  filter(
    months != "t1"
  ) |>
  glmer(formula = score ~ 1 + months * media * cohort + (1  | id) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum))

emmeans(fit.t.t2_split, pairwise~media|months|cohort, type='response')
plot_model(fit.t.t2_split, type='int')

##### without cohort nesting & compare 

fit.t1.v2 <- combined_data_cor_screened |>
  filter(
    months != "t2"
  ) |>
  glmer(formula = score ~ 1 + months * media   +  (1 | id) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum))


fit.t2.v2 <- combined_data_cor_screened |>
  filter(
    months != "t1"
  ) |>
  glmer(formula = score ~ 1 + months * media + (1  | id) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum))

anova(fit.t1, fit.t1.v2)
anova(fit.t2, fit.t2.v2)

##### with other covariates
fit.t1.cov <- combined_data_cor_screened |>
  filter(
    months != "t2"
  ) |>
  glmer(formula = score ~ 1 + months * media  + age + presence  + knew + ar_frequency +  (1 | id) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum))


fit.t2.cov <- combined_data_cor_screened |>
  filter(
    months != "t1"
  ) |>
  glmer(formula = score ~ 1 + months * media + age + presence  + knew + ar_frequency + (1  | id) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum))

##### because 'knew' is the predictor, shall we exclude people with super high 'knew'?

#####
fit.full <- glmer(score ~ 1 + age + presence  + knew + media * months  + media * ar_frequency +  (1  | exp_title / exp_title_question) + (1 | cohort / id)  ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), data=combined_data_cor_screened, contrasts = list(media = contr.sum, months=contr.sum))

# below, removing ar_frequency
fit.no_ar_frequency_int <- glmer(score ~ 1 + age + presence + enjoy + knew + media * months + media + ar_frequency + (1  | exp_title / exp_title_question) + (1 | cohort / id)  ,  family = binomial("logit"), data=combined_data_cor_screened, nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum, months=contr.sum))

anova(fit.full, fit.no_ar_frequency)

fit.no_months_int <- glmer(score ~ 1 + age + presence + enjoy + knew + media +  months + media + ar_frequency + (1  | exp_title / exp_title_question) + (1 | cohort / id)  ,  family = binomial("logit"), data=combined_data_cor_screened, nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum, months=contr.sum))
...
e2 = emmeans(fit.full, pairwise~media|ar_frequency, adjust = "sidak", type='response' )
confint(e2, adjust = "sidak", level = 0.99)



# below, combining advice from stackexchange, as well as some more predictors

fit.mod1 <-glmer(formula = score ~ 1 + age + ar_frequency + presence + knew + media * time * gap +  (1  | exp_title / exp_title_question) + (1 | cohort / id) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum, time=contr.sum, gap=contr.sum), data=combined_new)

### chekcing the impact of using : (where we dont nest but just )

fit.mod2 <-glmer(formula = score ~ 1 + age + ar_frequency + presence + knew + media * time * gap +  (1  | exp_title_question : exp_title) + (1 | id : cohort) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum, time=contr.sum, gap=contr.sum), data=combined_new)

### moving cohort out of random factors

fit.mod2 <-glmer(formula = score ~ 1 + age + ar_frequency + presence + knew + media * time * gap * cohort +  (1  | exp_title_question : exp_title) + (1 | id ) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum, time=contr.sum, gap=contr.sum), data=combined_new)

###### Marginal R2 / Conditional R2	0.030 / 0.254	0.051 / 0.257
###### note how we shift from conditional r2 to marginal r2, as expected

fit.mod2 <-glmer(formula = score ~ 1 + age + ar_frequency + presence + knew + media * time * gap * cohort +  (1  | exp_title_question : exp_title) + (1 | id ) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum, time=contr.sum, gap=contr.sum), data=combined_new)

### moving exp_title out of random factors

fit.mod2 <-glmer(formula = score ~ 1 + age + ar_frequency + presence + knew + media * time * gap * cohort * exp_title +   + (1 | id ) ,  family = binomial("logit"), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum, time=contr.sum, gap=contr.sum), data=combined_new)

###### Marginal R2 / Conditional R2	0.030 / 0.254	0.109 / 0.176
###### model fails to converge well but note the additional shift from conditional to marginal!!
```

# constructing combined measure that collapses exp_title_question
```{r}
count_combine <-combined_new |>
    group_by(media, months, exp_title, id, gap) |>
  dplyr::summarize(media, months, exp_title, id, time, av_score = sum(score), n=n(), gap, cohort) |>
  unique() |>
  filter(n==3) |>
  mutate(
    time = case_when(
...
count_combine |>
  arrange(desc(id))  |> print(n=24)

# nb counts 10370 / 11307 when we  have /no-not-have that filter

# Poisson or negative binomial regression

count_m1 <- glmer(formula = av_score ~ 1 + media * time * gap + (1 | id), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum, time=contr.sum, gap=contr.sum), data=count_combine, family = "poisson")
count_m2 <- glmer(formula = av_score ~ 1 + media * time * gap + (1 | cohort / id), nAGQ=0, control=glmerControl(optimizer = "nloptwrap"), contrasts = list(media = contr.sum, time=contr.sum, gap=contr.sum), data=count_combine, family = "poisson")



plot_model(count_m1, type='int')
